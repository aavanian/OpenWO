...
OpenWO is a minimal, no-account iOS app to track a personal gym routine built around 3 rotating workout types (A, B, C), a daily bodyweight challenge, and long-term habit streaks. All data is stored locally in a SQLite database synced to iCloud Drive. There is no backend, no authentication, and no network dependency.
...
```
<iCloud Container>/Documents/openwo.sqlite
```
